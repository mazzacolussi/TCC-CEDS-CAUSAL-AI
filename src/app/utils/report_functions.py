...
from app.config.settings import PROJECT_DIR
...
def run_iptw_analysis(
    ...
    print(f"Shape before preprocessing: {len(df):,}")
    df = preprocess(df, confounders, treatment, outcome)
    print(f"Shape after preprocessing: {len(df):,}")

    ps, weights, auc = compute_iptw_weights(
        df, confounders, treatment, trim_percentile=trim_percentile
    )
    print(f"\nAUC-ROC PS: {auc:.4f}")
    ...
    smd_antes = compute_smd(df, confounders, treatment)
    smd_depois = compute_smd(df, confounders, treatment, weights=weights)
    ...
    ate = ate_iptw(df, treatment, outcome, weights)
    ...
    axes[1].set_yticklabels(confounders, fontsize=8)
    axes[1].set_xlabel('SMD')
    axes[1].set_title('Balanco das covariaveis')
    axes[1].legend(fontsize=8)

    plt.tight_layout()
    plt.savefig(f'{PROJECT_DIR}/reports/figures/iptw_v10_{treatment}.png', dpi=150, bbox_inches='tight')
    plt.show()
    plt.close()
